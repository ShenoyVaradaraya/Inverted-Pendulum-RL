...
class Agent(object):
    def __init__(self, input_size=4, hidden_size=2, gamma=0.95,
        ...
        # Used the cartpole simulator from GYM openAI
        self.env = gym.make('CartPole-v0')
        # to save the simulation video, simply uncomment the line below
        ...
        # Parameters required for the controller
        ...
        #########################################################################################################
        # Single Neural network implementation for the control of the inverted pendulum. To include more layers and
        # check for the output uncomment below line which includes configuration of the hidden layers.
        #########################################################################################################
        # hidden_layer = layers.fully_connected(self.input_pl, hidden_size, biases_initializer=None, activation_fn=tf.nn.relu)
        # hidden_layer = layers.fully_connected(hidden_layer, hidden_size, biases_initializer=None, activation_fn=tf.nn.relu)
        ##########################################################################################################
        ...
        # to compute the loss of the output
        ...
        # check the gradient value with each iteration
    ...
    def video_callable(self, episode_id):
        # After 50 epochs this will run the episode of the in the video
        return episode_id % 50 == 0

    def next_action(self, sess, feed_dict, greedy=False):
        ######################################################################################
        #  According to current state determine the next action.                             #
        #  Args:                                                                             #
        ######################################################################################
        ...
        ######################################################################################
        #  Return: Integer, action to be taken.                                              #
        ######################################################################################
...
def one_trial(agent, sess, grad_buffer, reward_itr, i, render=False):
    ###########################################################################################
    # trial function requirements                                                             #
    ###########################################################################################
    # before a trial is done:                                                                 #
    ###########################################################################################
    # 1. define a sequence of action based on the current parameters and the control policy   #
    # 2. track the system response                                                            #
    ...
    while True:
        ...
        if done:
            ...
            # how long the simulation is done.
...
def get_fig(max_epoch):
    fig = plt.figure()
    ax_itr = plt.axes([0.1, 0.1, 0.8, 0.8])
    ax_obt = plt.axes([0.5, 0.2, .3, .3])
